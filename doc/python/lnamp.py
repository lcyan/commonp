...
apache_bak_url = 'httpd-2.2.27.tar.gz'
...
def install_dependence():
    if os_dist in('redhat', 'centos'):
        ...
        pkgs.extend(['libtidy-devel', 'patch', 're2c'])
...
def install_nginx():
    ...
    run_cmd("sed -i 's#Server: nginx#Server: %s#' src/http/ngx_http_header_filter_module.c" % nginx_banner)
    run_cmd("sed -i 's#\"nginx/\"#\"%s/\"#' src/core/nginx.h" % nginx_banner)
    run_cmd("sed -i 's#\"NGINX\"#\"%s\"#' src/core/nginx.h" % nginx_banner)
...
def install_php():
    ...
    # session
    system('sed -i "s#session.cookie_lifetime = 0#session.cookie_lifetime = 86400#g" /etc/php.ini')
    system('sed -i "s#session.gc_maxlifetime = 1440#session.gc_maxlifetime = 86400#g" /etc/php.ini')
...
def install_pma():
    ...
    file_append('libraries/config.default.php', "$cfg['LoginCookieValidity'] = 60*60*24*1;")
...
if __name__ == '__main__':
    ...
    parser.add_option("-b", "--banner", dest="banner", action="store",
                      help="Nginx Banner")
    ...
    # 指定nginx Banner中的名字
    if options.banner:
        nginx_banner = options.banner
    else:
        nginx_banner = "Server110 LNAMP"
